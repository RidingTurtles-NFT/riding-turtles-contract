...
// this contract is only for testing the max mint threshold in the FusionNft contract, it is an exact copy. only _maxNfts is set to 300

contract FusionNftMaxMintCheck is ERC721Enumerable, Ownable, TokenHolder {
    // events
    event PaymentReceived(address from, uint256 amount);
    event NftMinted(address to, uint16 tokenId);

    // constants
    uint16 private constant _maxNfts = 300;
    ...
    function generateNewTraits(address newOwner) internal {
        ...
        bytes32 traitsHash;
        for (uint i = 0; i < 3; i++) {
            uint8[] memory rand = _randGen.getNewTraits(newOwner, _tokenId + uint16(i) + uint16(newTraits.board));// random numbers between 0 and 255
            newTraits = Traits(
                1, // level
