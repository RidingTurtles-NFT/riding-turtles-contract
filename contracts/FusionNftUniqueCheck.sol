...
// this contract is only for testing the unique functionality of the mint function in the FusionNft contract, it is an exact copy. only generateNewTraits() is changed

contract FusionNftUniqueCheck is ERC721Enumerable, Ownable, TokenHolder {
    // events
    ...
    uint8[] private _traitVariations = [60, 7, 17, 17, 20, 35, 11, 11]; // maximum different types of each trait, without level (first) and special (last)
    ...
    // TODO: keep this special functionality simulating clashing traits, every first try in while(uniqueFound) gives same traits, every second gives random ones
    function generateNewTraits(address newOwner) internal {
        bool giveSame = true; // TODO: special for testing
        bool uniqueTraitHashFound = false;
        Traits memory newTraits;
        bytes32 traitsHash;
        for (uint i = 0; i < 3; i++) {
            uint8[] memory rand = _randGen.getNewTraits(newOwner, _tokenId + uint16(i) + uint16(newTraits.board));
            if (giveSame) { // TODO: special for testing
                newTraits = Traits(
                    1, // level
                    1,
                    1,
                ...
                newTraits = Traits(
                    ...
                    rand[5] < 29 ? 0 : rand[5] % _traitVariations[5], // ~15% // < (<TARGET_PERCENTAGE_OF_0>-1/<TRAIT_VARIATIONS>)*<MAX_RAND_IND> -> < (0,1-1/28)*255
                    rand[6] < 87 ? 0 : rand[6] % _traitVariations[6], // ~40%
                    rand[7] < 26 ? 0 : rand[7] % _traitVariations[7], // ~15%
                    0 // special
                );
            }
            giveSame = !giveSame;
    ...
    // TODO: keep this special functionality simulating clashing traits when fusing, every first try in while(uniqueFound) gives same traits, every second gives random ones
    function generateFusionTraits(address newOwner, uint16 firstTokenId, uint16 secondTokenId) internal {
        ...
        for (uint i = 0; i < 3; i++) {
            uint8[] memory newFusionTraits = _randGen.getFusionTraits(newOwner, _tokenId + uint16(i) + uint16(newTraits.board), firstArr, secondArr);
            if (giveSame) { // TODO: special for testing
                newTraits = Traits(
                    2, // level
                    1,
